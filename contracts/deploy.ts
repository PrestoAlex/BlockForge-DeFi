...
/**
 * BlockForge — Composable DeFi Lego Deployment Script
 *
 * Deployment flow:
 * 1. Deploy CoreContract
 * 2. Deploy each module contract (VaultModule, StakingModule, etc.)
 * 3. Register each module in CoreContract
 * 4. Set deposit/withdraw pipeline masks
 *
 * Usage:
 *   cp .env.example .env   # fill in your MNEMONIC
...
const wasmPath = process.env.WASM_PATH || './build/CoreContract.wasm';
...
async function main(): Promise<void> {
  ...
  console.log('=== BlockForge CoreContract Deployment ===');
  ...
  if (!confirmed) {
    ...
    try {
      ...
      if (code) {
        console.log('Bytecode verified! CoreContract is LIVE.');
      } else {
  ...
  console.log('\nNext steps:');
  console.log('1. Deploy module contracts (Vault/Staking/Rewards/TimeLock/Escrow)');
  console.log('2. Register each module via CoreContract.registerModule');
  console.log('3. Set custom pipelines via setDepositPipeline/setWithdrawPipeline');
  console.log(`\nSet window.BLOCKFORGE_CORE_CONTRACT = "${deployment.contractAddress}" in frontend.`);
}
